    ...
    constructor(model: Model) {
        return new Proxy(function () { }, {
            ...
            deleteProperty: (target: Data, key: string | number) => {
                ...
                if (model._connection) (async () => await model._database.forget(key))();
                model.deleted$.next({
                    type: 'deleted',
                    key: key,
                    value: null
            ...
            set: (target: Data, key: string | number, value, proxy) => {
                ...
                if (model._connection) (async () => await model._database.put(key, value))();
                model.updated$.next({
                    type: 'updated',
                    key: key,
                    value: value
            ...
            get: (target: Data, key: string | number, receiver) => {
                key = model.checkPrimary(key);
                if ('string' === typeof key && key.endsWith('$')) return model._cache.get$(key.substring(0, key.length - 1));

                return (async () => {
                    let result = model._cache.get(key, null);
                    if (model._connection && null === result) result = await model._database.get(key);
                    return result;
                })();
            },
            has: (target: Data, key: string | number) => {
                key = model.checkPrimary(key);
                if (model._connection) throw new Error(`For persistent models, the result of the "in" operator may be incorrect, need "await modelName.has(key)" replace your "key in modelName.data".`);
