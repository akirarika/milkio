...
export default class Model {
    ...
    constructor(connection: Connection | false, [primary, primaryType]: [string, string]) {
        ...
        if (connection && !connection.getConnection()[this.name]) throw new Error(`Table not found. Did you add this table '${this.name}' to the migrations?`);
    ...
    async getResults(query: Collection | Table, resultProt = {}) {
        if (!this._connection) throw new Error(`This function can only be used when you have Collection`);
        let querys = await query.toArray();
        if (!(querys instanceof Array)) throw new Error(`The query result is not a single object. If it's an array, please use 'getresult' instead.`);
    ...
    async getResult(query: Promise<Record<string | number, any>>, resultProt = {}) {
        if (!this._connection) throw new Error(`This function can only be used when you have Collection`);
        if (!this._isPlainObject(await query)) throw new Error(`The query result is not a single array. If it's an object, please use 'getresults' instead.`);
    ...
    checkPrimary(key) {
        if ("number" === this.primaryType && !isNaN(Number(key))) return Number(key);
        if (this.primaryType !== (typeof key)) throw new Error(`The model primary type needs to be ${this.primaryType}, not ${typeof key}`);
        return key;
    }
