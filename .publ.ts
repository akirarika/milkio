...
async function main() {
      ...
      for (const childPackage of [mainPackage, ...childPackages]) {
        if (
          ...
          if (childPackage === "vite-plugin-milkio")
            dependencies["vite-plugin-node"] = packageJson.dependencies["vite-plugin-node"];
          if (childPackage === "vite-plugin-milkio")
  ...
  if ((await cli.select("要编写发行说明吗？", ["否", "是"])) === "是") {
    ...
    while (true) {
      ...
      await writeFile(
        join(cwd, ".commands", "publish", "releases-english", `${newVersion}.md`),
