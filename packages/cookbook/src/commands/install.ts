...
export default await defineCookbookCommand(async (utils) => {
  ...
  if (params.commands.length !== 0) {
    ...
    if (project.value !== "<root>" && (params.commands.find((command) => {
      ...
      consola.warn("You seem to be installing milkio-related packages or some common public packages. We recommend that you install these packages in the root directory so that you can upgrade all monorepo packages uniformly. Otherwise, it may be painful for you to upgrade these public package versions for each monorepo in the future.")
