...
export const getActionOptions = (options: string) => {
    const checkResult = (() => { const $join = (typia.misc.validatePrune as any).join; const $io0 = (input: any): boolean => "object" === typeof input.projects && null !== input.projects && false === Array.isArray(input.projects) && $io1(input.projects) && ("object" === typeof input.general && null !== input.general && $io3(input.general)); const $io1 = (input: any): boolean => Object.keys(input).every((key: any) => {
        const value = input[key];
        if (undefined === value)
            return true;
        return "object" === typeof value && null !== value && $io2(value);
    }); const $io2 = (input: any): boolean => ("milkio" === input.type || "other" === input.type) && "number" === typeof input.port && (Array.isArray(input.start) && input.start.every((elem: any) => "string" === typeof elem)) && (Array.isArray(input.build) && input.build.every((elem: any) => "string" === typeof elem)) && (undefined === input.lazyRoutes || "boolean" === typeof input.lazyRoutes) && (undefined === input.typiaMode || "generation" === input.typiaMode || "bundler" === input.typiaMode) && (undefined === input.significant || Array.isArray(input.significant) && input.significant.every((elem: any) => "string" === typeof elem)) && (undefined === input.insignificant || Array.isArray(input.insignificant) && input.insignificant.every((elem: any) => "string" === typeof elem)); const $io3 = (input: any): boolean => "number" === typeof input.cookbookPort; const $vo0 = (input: any, _path: string, _exceptionable: boolean = true): boolean => [("object" === typeof input.projects && null !== input.projects && false === Array.isArray(input.projects) || $report(_exceptionable, {
            path: _path + ".projects",
            expected: "Record<string, __type>",
            value: input.projects
        })) && $vo1(input.projects, _path + ".projects", true && _exceptionable) || $report(_exceptionable, {
            ...
            expected: "__type.o1",
            value: input.general
        })].every((flag: boolean) => flag); const $vo1 = (input: any, _path: string, _exceptionable: boolean = true): boolean => [false === _exceptionable || Object.keys(input).map((key: any) => {
            const value = input[key];
            if (undefined === value)
                return true;
            return ("object" === typeof value && null !== value || $report(_exceptionable, {
            ...
            expected: "(Array<string> | undefined)",
            value: input.insignificant
        })].every((flag: boolean) => flag); const $vo3 = (input: any, _path: string, _exceptionable: boolean = true): boolean => ["number" === typeof input.cookbookPort || $report(_exceptionable, {
            path: _path + ".cookbookPort",
            expected: "number",
            value: input.cookbookPort
        })].every((flag: boolean) => flag); const $po0 = (input: any): any => {
        if ("object" === typeof input.projects && null !== input.projects)
            $po1(input.projects);
        if ("object" === typeof input.general && null !== input.general)
            $po3(input.general);
        for (const key of Object.keys(input)) {
            if ("projects" === key || "general" === key)
                continue;
            delete input[key];
        }
        ...
        for (const key of Object.keys(input)) {
            if ("type" === key || "port" === key || "start" === key || "build" === key || "lazyRoutes" === key || "typiaMode" === key || "significant" === key || "insignificant" === key)
                continue;
            delete input[key];
        }
    ...
    }; const __is = (input: any): input is CookbookOptions => "object" === typeof input && null !== input && $io0(input); let errors: any; let $report: any; const __validate = (input: any): typia.IValidation<CookbookOptions> => {
        if (false === __is(input)) {
            errors = [];
    ...
    }; const __prune = (input: CookbookOptions): void => {
