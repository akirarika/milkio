...
export default await defineCookbookCommand(async (utils) => {
  ...
  if (
    !(await exists(join(cwd, ".commands", "publish"))) ||
    !(await exists(join(cwd, ".commands", "publish", "publish.json")))
  ) {
    ...
    if (!(await exists(join(cwd, ".commands", "publish"))))
      await mkdir(join(cwd, ".commands", "publish"));
    if (!(await exists(join(cwd, ".commands", "publish", "releases"))))
      await mkdir(join(cwd, "releases"));
    if (!(await exists(join(cwd, ".commands", "publish", "releases-english"))))
      await mkdir(join(cwd, "releases-english"));
    await writeFile(
      join(cwd, ".commands", "publish", "publish.json"),
  ...
  if ((await $`git status --porcelain`.text()).trim()) {
    ...
    // @ts-ignore
    await utils.gotoGitCommitCommand();
  }
  ...
  const publishJson = JSON.parse(
    await readFile(join(cwd, ".commands", "publish", "publish.json"), "utf-8")
  );
  ...
  try {
    console.log(
      `检查 npm 版本是否存在.. npm view ${packageJson.name}@${newVersion} --json`
    );
    ...
    if (
      ...
      if (checkGitStatus.trim() !== "") {
        ...
        await $`git push -u origin ${(await $`git symbolic-ref --short HEAD`)
          .text()
          .trim()}`;
      }
      ...
      await (async () => {
        ...
        if (!existsSync("./packages/cookbook/dist"))
          await mkdir("./packages/cookbook/dist");
        const packageJson = JSON.parse(
          await readFile("./packages/cookbook/package.json", "utf-8")
        );

        for (const platform of platforms) {
          consola.log(
            `正在打包 ${platform.platform} ${platform.arch} 的二进制文件..`
          );
          ...
          await writeFile(
            `./packages/cookbook/dist/cookbook-${platform.platform}-${platform.arch}/index.js`,
      ...
      for (const childPackage of [mainPackage, ...childPackages]) {
        if (
          childPackage !== "cookbook" &&
          childPackage !== "cookbook-ui" &&
          childPackage !== "create-cookbook"
        ) {
          ...
          if (childPackage === "vite-plugin-milkio")
            external.push("vite-plugin-node");
          if (childPackage === "vite-plugin-milkio")
            external.push("@mjackson/node-fetch-server");
          ...
          try {
            await $`bun ../../node_modules/typescript/bin/tsc index.ts --declaration --emitDeclarationOnly --outDir ./dist --module nodenext --moduleResolution nodenext --allowImportingTsExtensions`
              .cwd(join(cwd, "packages", childPackage))
              .quiet();
          } catch (error) {}
          await Bun.write(
            join(cwd, "packages", childPackage, "dist", "LICENSE"),
            await Bun.file(join(cwd, "LICENSE")).text()
          );
          const packageJson = JSON.parse(
            await readFile(
              join(cwd, "packages", childPackage, "package.json"),
          ...
          if (childPackage === "milkio")
            dependencies["@southern-aurora/tson"] = "*";
          if (childPackage === "vite-plugin-milkio")
            dependencies["vite-plugin-node"] =
              packageJson.dependencies["vite-plugin-node"];
          if (childPackage === "vite-plugin-milkio")
            dependencies["@mjackson/node-fetch-server"] =
              packageJson.dependencies["@mjackson/node-fetch-server"];
          ...
          while (true) {
            try {
              ...
              await $`${{ raw: command }}`.cwd(
                join(cwd, "packages", childPackage, "dist")
              );
          ...
          while (true) {
            try {
              ...
              await $`${{ raw: command }}`.cwd(
                join(cwd, "packages", childPackage)
              );
              await Bun.sleep(1000);
              break;
            } catch (error: any) {
  ...
  consola.success(
    `所有的包均发布完成：${[mainPackage, ...childPackages].join(", ")}`
  );
  ...
  if ((await cli.select("要编写发行说明吗？", ["否", "是"])) === "是") {
    ...
    if (
      await exists(
        join(cwd, ".commands", "publish", "releases", `${newVersion}.md`)
      )
    ) {
      console.log(
        "已存在该版本的发布说明文件，你可能输入了一个已经存在的版本号"
      );
      if ((await cli.select("确定使用此版本吗？", ["否", "是"])) === "否")
        process.exit(0);
      console.log(
        "请编辑发行说明文件，并在编辑完成后，再继续操作 (VS Code 按住 Ctrl 键点击下方路径可快速编辑)\n"
      );
    } else {
      ...
      console.log(
        "已创建发行说明文件。请编辑它，并在编辑完成后，再继续操作 (VS Code 按住 Ctrl 键点击下方路径可快速编辑)\n"
      );
    }
    ...
    while (true) {
      ...
      if (releaseNote.includes("一段编纂出来的虚构科幻小说的摘抄片段")) {
        console.log(
          `含有 "一段编纂出来的虚构科幻小说的摘抄片段" 内容，校验未通过，请重新编辑它`
        );
        alert("按回车以继续");
        continue;
      }
    ...
    while (true) {
      ...
      const releaseGithubNoteTranslated = await translateToEnglish(
        releaseGithubNote
      );
      await writeFile(
        join(
      ...
      console.log(
        "其中，还重新回译了一份译文，你可以结合译文和回译译文，来判断结果是否有错漏。在正式提交前，需删除回译的译文\n"
      );
      console.log(
        join(
          cwd,
    ...
    while (true) {
      ...
      releaseGithubNote = await readFile(
        join(
          cwd,
          ".commands",
          "publish",
    ...
    await octokit.request(
      `POST /repos/${publishJson.githubOwner}/${publishJson.githubRepo}/releases`,
      {
    ...
    await fetch(
      `https://gitee.com/api/v5/repos/${publishJson.giteeOwner}/${publishJson.giteeRepo}/releases`,
      {
        method: "POST",
        body: (() => {
  ...
  console.log("\n\n发布成功\n");
  console.log(
    `- npm: https://www.npmjs.com/package/${packageJson.name}/v/${newVersion}`
  );
  console.log(
    `- gitee: https://gitee.com/${publishJson.giteeOwner}/${publishJson.giteeRepo}/releases/tag/v${newVersion}`
  );
  console.log(
    `- github: https://github.com/${publishJson.githubOwner}/${publishJson.githubRepo}/releases/tag/v${newVersion}`
  );
